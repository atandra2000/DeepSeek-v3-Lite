# utils/logging.py
"""
Single-GPU training logger with optional WandB integration.

WandB is enabled by setting the environment variable `WANDB_PROJECT`.
"""
import json
import os
import time
...
class TrainingLogger:
    """
    Step-driven training logger. Prints a rolling-window summary to stdout
    every `log_interval` steps; optionally forwards to Weights & Biases.
    """
...
def init_logging(
    ...
    """Initialise the global training logger."""
    global _logger
    _logger = TrainingLogger(log_interval=log_interval, seq_len=seq_len)


def get_logger() -> TrainingLogger:
    """Return the global logger, initialising on first call if needed."""
    global _logger
    if _logger is None:
