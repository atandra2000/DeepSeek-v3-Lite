...
Supports:
  • Standard autoregressive decoding with KV cache (temperature / top-p sampling)
...
# ── Standard generation (KV-cache aware) ───────────────────────────────────────
...
def generate_tokens(
    ...
    """
    KV-cache autoregressive generation. Delegates to `Transformer.generate`
    so the same code path is used in both production and inference scripts.
    """
...
def main():
    ...
    # Optional MTP draft head. MTP weights, if present, are stored in the
    # main safetensors file under the "mtp" prefix (see CheckpointManager
    # round-trip). If absent we still build the module so speculative
    # decoding can be enabled, but the head will produce untrained drafts.
    mtp_module: Optional[MTPModule] = None
    if args.use_speculative:
        mtp_module = MTPModule(model_cfg, depth=1).to("cuda")
        mtp_module.eval()
        # Try to load an mtp substate if the checkpoint contains one.
